...
float SETPOINT = 35.0;   // Temperatura alvo em °C
double x;
...
// ========== CONFIGURAÇÕES ==========
const int PWM_PIN = 9;             // Pino para saída PWM (0-255)
const int PWM_PIN_OFF = 3; 
...
void setup() {
  ...
  // Inicializa comunicação com Arduino Cloud
...
void loop() {
  ...
  if (currentMillis - previousMillis >= SAMPLE_RATE_MS) {
    ...
    x = ((SETPOINT - filteredTemperature)*500)+127;
    pwmValue = constrain((int)x, 0, 255);
    // 6. Converter saída para PWM
    //pwmValue = constrain((int)y, 0, 255);
    analogWrite(PWM_PIN, pwmValue);
...
float movingAverageFilter(float newSample) {
  ...
  for (int i = 0; i < NUM_SAMPLES; i++) {
    if (samples[i] > -50 && samples[i] < 500) {  // Filtro de valores plausíveis
      sum += samples[i];
      validSamples++;
    }
  }
